...
public class PreferencesProviderWrapper {
	...
	private static final String TAG = "SIP Prefs";
	...
	private boolean isValidWifiConnectionFor(NetworkInfo ni, String suffix) {
		...
		boolean valid_for_wifi = getPreferenceBooleanValue(
				"use_wifi_" + suffix, true);
	...
	private boolean isValidMobileConnectionFor(NetworkInfo ni, String suffix) {
		boolean valid_for_3g = getPreferenceBooleanValue("use_3g_" + suffix,
				false);
		boolean valid_for_edge = getPreferenceBooleanValue(
				"use_edge_" + suffix, false);
		boolean valid_for_gprs = getPreferenceBooleanValue(
				"use_gprs_" + suffix, false);
		boolean valid_for_roaming = getPreferenceBooleanValue("use_roaming_"
				+ suffix, true);
	...
	private boolean isValidOtherConnectionFor(NetworkInfo ni, String suffix) {
		...
		boolean valid_for_other = getPreferenceBooleanValue("use_other_"
				+ suffix, true);
		// boolean valid_for_other = true;
		if (valid_for_other && ni != null
	...
	private boolean isValidConnectionFor(NetworkInfo ni, String suffix) {
		if (isValidWifiConnectionFor(ni, suffix)) {
			Log.d(TAG, "We are valid for WIFI");
			return true;
		}
		if (isValidMobileConnectionFor(ni, suffix)) {
			Log.d(TAG, "We are valid for MOBILE");
			return true;
		}
		if (isValidOtherConnectionFor(ni, suffix)) {
			Log.d(TAG, "We are valid for OTHER");
			return true;
		}
		if (isValidAnywayConnectionFor(ni, suffix)) {
			Log.d(TAG, "We are valid ANYWAY");
			return true;
		}
		return false;
	}
	 ...
	 * 
	 * @param considerQuit
	 *            pass true if we should consider app quitted as a reason to not
	 *            consider available for outgoing
	 * @return true if connection is valid
	 */
	public boolean isValidConnectionForOutgoing(boolean considerQuit) {
		if (considerQuit) {
			if (getPreferenceBooleanValue(PreferencesWrapper.HAS_BEEN_QUIT,
					false)) {
	...
	public int getInCallMode() {
		...
		try {
			...
			Log.e(TAG, "In call mode " + mode + " not well formated");
		}
	...
	public long getClockRate(MediaManager mediaManager) {
		...
		try {
			...
			Log.e(TAG, "Clock rate " + clockRate + " not well formated");
		}
		return defaultRate;
	}
	...
	public float getInitialVolumeLevel() {
		return (float) (getPreferenceFloatValue(
				SipConfigManager.SND_STREAM_LEVEL, 8.0f) / 10.0f);
	}
	...
	public int getTcpKeepAliveInterval() {
		return getKeepAliveInterval(
				SipConfigManager.TCP_KEEP_ALIVE_INTERVAL_WIFI,
	...
	public int getTlsKeepAliveInterval() {
		return getKeepAliveInterval(
				SipConfigManager.TLS_KEEP_ALIVE_INTERVAL_WIFI,
	...
	public String getUserAgent(Context ctx) {
		...
		if (userAgent.equalsIgnoreCase(CustomDistribution.getUserAgent())) {
			...
			if (pinfo != null) {
				userAgent += "_" + android.os.Build.DEVICE + "-"
						+ Compatibility.getApiLevel() + "/r"
						+ pinfo.versionCode;
			}
		}
		return userAgent;
	...
	public long getMediaQuality() {
		...
		try {
			...
			Log.e(TAG, "Audio quality " + mediaQuality
					+ " not well formated");
		}
	...
	public void initCodecPriorityList(List<String> codecs) {
		String type = "wb";
		int i = 1;
		for (String codecName:codecs) {
			if (codecName.contains("PCMU") && codecName.contains("8000")) {
				String key = SipConfigManager.getCodecKey(codecName, type);
				setPreferenceStringValue(key, i+ "");
				Log.d("henry2" ,"key = " + key);;
			}
		}
	}
	// Codecs
	 ...
	 * 
	 * @param codecName
	 *            codec name formated in the pjsip format (the corresponding
	 *            pref is codec_{{lower(codecName)}}_{{codecFreq}})
	 * @param defaultValue
	 *            the default value if the pref is not found MUST be casteable
	 *            as Integer/short
	 * @return the priority of the codec as defined in preferences
	 */
	public short getCodecPriority(String codecName, String type,
			String defaultValue) {
		String key = SipConfigManager.getCodecKey(codecName, type);
		Log.d(TAG,"key = " + key);
		if (key != null) {
			...
			if (!TextUtils.isEmpty(val)) {
				try {
					...
					Log.e(TAG, "Impossible to parse " + val);
				}
			}
		}
	 ...
	 * 
	 * @param codecName
	 *            the name of the codec as announced by codec
	 * @param type
	 *            bandwidth type <br/>
	 ...
	 * @param newValue
	 *            Short value for preference as a string.
	 */
